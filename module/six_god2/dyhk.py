...
class VerifyAweme:
    ...
    def captcha(self, verify_data):
        ...
        try:
            response = requests.get(url=url, headers=self._headers,proxies=self.proxy,timeout=30).json()
            # print(response)
        ...
        try:
            binary_ct = BytesIO(requests.get(content['question']['url2'],proxies=self.proxy,timeout=30).content)
            binary_bg = BytesIO(requests.get(content['question']['url1'],proxies=self.proxy,timeout=30).content)
        except requests.exceptions.RequestException as e:
            print(f"代理无效: {e}")
            return(json.dumps({
                'code': -2,
        ...
        try:
            result = requests.post(url, json=payload, headers=self._headers,proxies=self.proxy,timeout=30).json()
            logger.debug(f"滑块响应内容:{result}", )
            return result
        except requests.exceptions.RequestException as e:
            logger.error(f"代理无效: {e}")
            return(json.dumps({
                'code': -2,
                'msg': '代理IP失效'
            }))
