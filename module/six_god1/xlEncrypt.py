...
def get_xladon(xk,app_info):
    randBytes = struct.pack("<I", random.randint(0x10000000, 0xFFFFFFFF))
    key = generateKey(randBytes, app_info['aid'])
    cipher = SpeckCipher(key, 256, 128, 'ECB')
    plainText = pad((str(xk) + "-1588093228-" + app_info['aid']).encode(), 16)
    cipherText = cipher.encrypt(plainText[:16]) + cipher.encrypt(plainText[16:])
    return base64.b64encode(randBytes + cipherText).decode()
