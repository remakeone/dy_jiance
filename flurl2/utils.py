...
def get_proxy(proxys=None):
    if proxys is None:
        proxy_str = None
    else:
        proxy_ip = proxys.get('ip')
        proxy_port = proxys.get('port')
        proxy_username = proxys.get('key')
        proxy_password = proxys.get('pwd')

        if proxy_username and proxy_password and proxy_ip and proxy_port:
            proxy_str = f"http://{proxy_username}:{proxy_password}@{proxy_ip}:{proxy_port}"
